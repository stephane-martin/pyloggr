define(
    ...
    ws.onmessage = function (evt) {
        ...
        if (r.action == 'queues.stats') {
            React.render(<RabbitMQ {...r} />, document.getElementById('list_of_rabbitmq_queues'));
        } else if (r.action == 'status') {
            React.render(<OnOffStatus {...r} />, document.getElementById('list_of_status'));
        } else if (r.action == 'syslogs') {
            React.render(<SyslogServers {...r} />, document.getElementById('list_of_syslog_servers'));
        } else if (r.action == 'pgsql.stats') {
            React.render(<PGSQL {...r} />, document.getElementById('list_of_pgsql_stats'));
        } else {
            alert(JSON.stringify(r, null, 4));
        }
    };
